...
// @notice credits to https://github.com/worldcoin/world-id-example-airdrop/blob/main/src/Semaphore.sol . 


import { Verifier } from '@appliedzkp/semaphore-contracts/base/Verifier.sol';
import { ISemaphore } from '@appliedzkp/semaphore-contracts/interfaces/ISemaphore.sol';
import { SemaphoreCore } from '@appliedzkp/semaphore-contracts/base/SemaphoreCore.sol';
import { SemaphoreGroups } from '@appliedzkp/semaphore-contracts/base/SemaphoreGroups.sol';


import { IncrementalBinaryTree,
    IncrementalTreeData } from '@zk-kit/incremental-merkle-tree.sol/contracts/IncrementalBinaryTree.sol';
...
contract Semaphore is ISemaphore, SemaphoreCore, Verifier, SemaphoreGroups {
    ...
    /// @notice Thrown when trying to update or create  DAO groups without being the manager.
    error Unauthorized();

    /// @notice Thrown when trying to create a group for DAO  with id   not in the range , since this can later cause issues with root history verification.
    error InvalidId();

    /// @notice Thrown when attempting to validate a root that doesn't belong to the specified group for governance DAO.
    error InvalidRoot();
    ...
    /// @notice Create a new identity group for the given DAO account. Can only be called by the issuer of the given DAO.
    ...
    /// @notice Add a new member to an existing group for the whitelisted in the DAO group. Can only be called by the creator of the DAO
    /// @param groupId The id of the group
    ...
    /// @param root  Merkle tree root id.
    ...
    function verifyProof(
        ...
        //TODO: change  the parameters to saitsfy the use case for  the XDAO pod creation and addingEntity
